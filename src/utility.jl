...
function upper_bound_sum_subtree_sizes(search_state, expansion=nothing) :: Float32
    ...
    sum(m -> expr_of_match(m).metadata.size, matches)
end
...
function upper_bound_with_conflicts(search_state, expansion=nothing) :: Float32
    matches = if isnothing(expansion) search_state.matches else expansion.matches end
    issorted(matches, by=m -> expr_of_match(m).metadata.id) || error("matches is not sorted")

    bound = 0.
    offset = length(matches)

    while true
        bound += expr_of_match(matches[offset]).metadata.size
        ...
        # things.
        next_id = expr_of_match(matches[offset]).metadata.id - expr_of_match(matches[offset]).metadata.num_nodes
        next_id == 0 && break
        ...
        offset -= 1
        offset == 0 && break
        expr_of_match(matches[offset]).metadata.id <= next_id && continue

        # rarer case: run binary search to find the rightmost non-child of the previous match
        offset = searchsortedlast(matches, search_state.all_nodes[next_id].match, by=m -> expr_of_match(m).metadata.id)
        offset == 0 && break
    end
    bound
end
